 ...
 */
export async function waitForNewAttestationEvent(api, attestationId, emitter) {
    if (!attestationId) {
    ...
    return new Promise((resolve, reject) => {
        const internalEmitter = subscribeToNewAttestations(api, () => { }, attestationId);
...
export function checkReadOnly(readOnly) {
    if (readOnly) {
 ...
 */
export function validateProofVersion(proofType, version) {
    const config = proofConfigurations[proofType];
    if (config.supportedVersions.length > 0) {
        ...
        if (!config.supportedVersions.includes(version)) {
            throw new Error(`Invalid version '${version}' for proof type: ${proofType}. Supported versions: ${config.supportedVersions.join(', ')}`);
        }
    }
}
