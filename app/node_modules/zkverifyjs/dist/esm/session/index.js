...
import { createSubmitProofExtrinsic, createExtrinsicHex, createExtrinsicFromHex, } from '../api/extrinsic';
...
export class zkVerifySession {
     ...
     */
    constructor(connection, customNetwork = false) {
        ...
        for (const network in SupportedNetwork) {
            if (Object.prototype.hasOwnProperty.call(SupportedNetwork, network)) {
                builderMethods[network] = (customWsUrl) => {
                    return new NetworkBuilder(zkVerifySession._startSession.bind(zkVerifySession), SupportedNetwork[network], customWsUrl);
                };
            }
        }
    ...
    createRegisterKeyBuilder(proofOptions) {
        return new RegisterKeyBuilder(this.executeRegisterVerificationKey.bind(this), proofOptions);
    }
    /**
    ...
    async executeVerify(options, input) {
        ...
        const transactionResult = verify(this.connection, options, events, input);
    ...
    async executeOptimisticVerify(proofOptions, input) {
        ...
        return optimisticVerify(this.connection, proofOptions, input);
    }
    /**
    ...
    async executeRegisterVerificationKey(options, verificationKey) {
        checkReadOnly(this.readOnly);
        return registerVk(this.connection, options, verificationKey);
    }
    /**
    ...
    async estimateCost(extrinsic) {
        checkReadOnly(this.readOnly);
        return estimateCost(this.connection.api, extrinsic, this.connection);
    }
    /**
    ...
    async format(proofOptions, proof, publicSignals, vk, version, registeredVk) {
        return format(proofOptions, proof, publicSignals, vk, version, registeredVk);
    }
    /**
    ...
    async accountInfo() {
        checkReadOnly(this.readOnly);
        return accountInfo(this.connection.api, this.connection.account);
    }
    /**
