 ...
 */
export const createSubmitProofExtrinsic = (api, proofType, params) => {
    const pallet = getProofPallet(proofType);
    if (!pallet) {
        throw new Error(`Unsupported proof type: ${proofType}`);
    }
    try {
        return api.tx[pallet].submitProof(params.formattedVk, params.formattedProof, params.formattedPubs, null);
    }
    catch (error) {
 ...
 * @throws {Error} - Throws an error if the extrinsic creation fails.
 */
export const createExtrinsicHex = (api, proofType, params) => {
 ...
 */
export const createExtrinsicFromHex = (api, extrinsicHex) => {
    try {
        return api.createType('Extrinsic', hexToU8a(extrinsicHex));
    }
    catch (error) {
